# Hi, I'm Akshat Kumar Sharma 👋

🚀 Robotics Software Developer (Future Goal) | AI & ML Enthusiast | Full-Stack Developer

I'm a Computer Science student at Career Point University, Kota, with experience in software development, web development, machine learning, and AI-powered applications. I enjoy building scalable solutions, exploring emerging technologies, and solving real-world problems through code.

## 💻 Technical Skills
- Robotics: python ,opencv ,ROS2, Gajebo
- Languages: Python, Java, C, C++, JavaScript
- Web Development: MERN Stack, Flask
- AI/ML: TensorFlow, PyTorch, Scikit-learn, NumPy
- Databases: MySQL, PostgreSQL, MongoDB, Firebase, Supabase
- Tools: Git, GitHub, VS Code, PyCharm, Google Colab

## 🚀 Projects
- 📧 Email Spam Detection using NLP and Machine Learning
- 🤖 AI Chatbot powered by Transformer Models
- 🔊 Text-to-Speech Synthesis System
- 🎨 AI Image Generation Applications

## 💼 Experience
- Game Developer Intern @ Akhiyan Company
- Web Development Intern @ GAO Tek Inc.
- Software Development Intern @ Qsarti Computers
- Artificial Intelligence Engineer Intern @ Samyak Computers

## 📚 Certifications
- Oracle Cloud Infrastructure 2025 Certified Generative AI Professional
- Data Science Engineer (DSE)
- Project Management Tech (PMT)
- OpenCV Bootcamp
- Tata GenAI Powered Data Analytics Simulation

## 🌱 Currently Learning
- Advanced AI & LLM Applications
- Full-Stack Development
- System Design & Cloud Technologies

## 📫 Connect With Me
- LinkedIn: www.linkedin.com/in/a-kumarsharma101
- Portfolio: aksh-portfolio.surge.sh

---
⭐ Always open to collaboration on AI, Machine Learning, and Full-Stack Development projects.
